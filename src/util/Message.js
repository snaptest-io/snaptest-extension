...
var Message = {
  ...
  toWindow: function(action, payload = {}, cb = this.NOOP) {
    if (destination === this.TAB) {
      chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        chrome.tabs.sendMessage(tabs[0].id, {action: action, payload: payload, destination: destination}, function(response) {
          cb(response);
  ...
  promiseToPanel: (action, payload) => {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendMessage({action, payload: {...payload, type: "PROMISEACTION"}, destination: "PANEL"}, function(response) {
        if (response.success) {
  ...
  onMessageForAsync: function(destination, cb = this.NOOP ) {
    chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
      if (request.destination === destination || request.destination === "all") {
        ...
        // cb(request, sender).then((r) => sendResponse(r))
        // return cb(request, sender, sendResponse);
    });
  },
