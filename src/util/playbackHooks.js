...
window.checkElementVisible = function (action) {
  ...
  if (elem) {
    function isVisible(elem) {
      ...
      if (action.checkVisibility && style.visibility !== "visible")
        return false;
...
window.getElementCount = function (action) {
  var elements;

  if (action.selectorType === SEL_CSS) {
    elements = document.querySelectorAll(action.selector);
  } else if (action.selectorType === SEL_XPATH) {
    ...
    elements = { length: result.snapshotLength };
  } else if (action.selectorType === SEL_ID) {
    var element = document.getElementById(action.selector);
    elements = element ? [element] : [];
  } else if (action.selectorType === SEL_NAME) {
    elements = document.getElementsByName(action.selector);
  } else {
    elements = document.querySelectorAll(action.selector);
  }
...
window.getElementAttribute = function (action) {
  var element = getElement(action);
  if (!element) {
    return { success: false, value: "" };
  }
  var value = element.getAttribute(action.attribute) || "";
  return { success: true, value };
};
...
window.setDialogs = function (action) {
  ...
  if (action.alert) element.setAttribute("data-alert", "true");
  ...
  if (action.prompt) element.setAttribute("data-prompt", action.promptResponse);
...
window.executeScript = function (action) {
  try {
    ...
    if (_.isBoolean(result) && !result) {
      return {
        success: false,
        message: "Script failed because it returned false.",
        value: result,
      };
    } else {
      return { success: true };
