...
class ActionSelectorDD extends React.Component {
        ...
        <div className="dropdown-box as-keepopen">
          <div className="grid-row">
            <div>
              <div
                className="search"
                onMouseEnter={() => this.setState({ revealDetailsFor: null })}
              >
                <input
                  type="text"
                  value={searchValue}
              ...
              {searchValue.length > 0 && searchResults.length > 0 ? (
                  ...
                  {searchResults.map((result, idx) => (
                    <div
                      className={
                        ...
                        (selectedResult === result.constant ? " selected" : "")
                      }
                      key={idx}
                      onClick={() =>
                        result.disabled
                          ? null
                          : onActionSelected(result.constant)
                      }
                    >
                ...
                <div className="search-results">
                  ...
                  <div className="no-result-suggestion">
                    Try topics like "form", "mouse", "navigation" etc...
                  </div>
                </div>
              ) : (
                <div className="grid-row">
                  <div
                    ...
                    onMouseEnter={() =>
                      this.setState({ revealDetailsFor: null })
                    }
                  >
                    {ActionDefs.ActionsByCategory.map((category, idx) => (
                      <div
                        ...
                        key={idx}
                        onMouseEnter={() =>
                          this.onMouseEnterCategory(category.label)
                        }
                        onMouseLeave={() =>
                          this.onMouseLeaveCategory(category.label)
                        }
                      >
                    ...
                    <div
                      ...
                      onMouseEnter={() =>
                        this.onMouseEnterCategory("Components")
                      }
                      onMouseLeave={() =>
                        this.onMouseLeaveCategory("Components")
                      }
                    >
                      <div>Components</div>
                    </div>
                  ...
                  {selectedCategory === "Components" ? (
                      ...
                      {components
                        ...
                        .map((component) => (
                          <div
                            className="component"
                            onClick={() =>
                              onActionSelected("COMPONENT", component.id)
                            }
                          >
                            <div className="name">{component.name}</div>
                            <div className="path">
                              {buildPathString(component, tree)}
                            </div>
                          </div>
                        ))}
                    </div>
                    ...
                    <div className="components">
                      {tests
                        .filter((test) => test.type === "request")
                        .map((request) => (
                          <div
                            className="component"
                            onClick={() =>
                              onActionSelected("REQUEST", request.id)
                            }
                          >
                    ...
                    <div className="actions">
                      {ActionDefs.ActionsByCategoryMap[
                        selectedCategory
                      ].actions.map((act, idx) => (
                        <div
                          className={
                            "action" + (act.disabled ? " disabled" : "")
                          }
                          key={idx}
                          onMouseEnter={() =>
                            this.setState({ revealDetailsFor: act.constant })
                          }
                          onClick={() =>
                            act.disabled ? null : onActionSelected(act.constant)
                          }
                        >
                          {act.name}
                        </div>
  ...
  onKeydown(e) {
    ...
    if (e.code === "ArrowDown") {
      e.preventDefault();
      if (resultsByConstant[idxOfSelectedResult + 1])
        this.setState({
          selectedResult: resultsByConstant[idxOfSelectedResult + 1],
        });
