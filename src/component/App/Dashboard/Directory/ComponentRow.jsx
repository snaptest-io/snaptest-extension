...
class ComponentRow extends React.Component {
  ...
  render() {
    ...
    const { connectDragSource, node, isDragging, component, connectDragPreview, active = false } = this.props;
    ...
    return (
      <RowMonitor nodeId={node.id} className="dir-item-row dir-test-row">
          <div className={dirItemClassNames}>
            {connectDragSource(
              <div className={clickHandleClassnames} onClick={() => this.onViewComp(component)}>
                {connectDragPreview(
  ...
  onViewComp(component) {
    Message.to(Message.SESSION, "setComponentActive", component.id);
    Message.to(Message.SESSION, "newRouteStack", new Route("componentbuilder", { componentId: component.id } ) );
  }
