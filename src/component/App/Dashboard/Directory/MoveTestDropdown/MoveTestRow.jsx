...
class MoveTestRow extends React.Component {
    ...
    return (
      <div className={classNames} onClick={(e) => this.onClick(e) }>
        {processing ? (
          ...
          <img src={chrome.runtime.getURL("assets/cloudmove.png")} />
        ) : (
  ...
  onClick(e) {
    ...
    const { test, type, id } = this.props;

    this.setState({processing: true, success: false, error: null});

    Message.promise("copyTestToAccount", {testId: test.id, contextId: id, contextType: type})
      .then(() => {
